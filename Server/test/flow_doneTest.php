...
class flow_doneTest extends PHPUnit_Framework_TestCase
{
    ...
    /**
     * @depends testflowCheckorder
     */
    function testflowDone($data)
    {
        $session = $data['session'];
        $flow = $data['data'];
        ...
        $data = array(
            'session' => $session,
            'pay_id' => $data['data']['payment_list'][0]['pay_id'],
            'shipping_id' => $data['data']['shipping_list'][0]['shipping_id'],
        );
        $out = request_by_curl('flow/done', $data);
        $this->assertArrayHasKey('order_id', $out['data']);
    }
}